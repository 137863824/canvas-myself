$(function(){
	...
	/*cak*/
	// 公共导航
	$('.link-1').on('mouseleave',function () {
		$(".link-1").slideUp(100);
	});
	$('.link-2').on('mouseleave',function () {
		$(".link-2").slideUp(100);
	});
	$('.right-top').on('mouseover',function(event){
		event.stopPropagation();
		$(".link-1").slideDown(100);
	})
	// $('.link-1').on('click','a',function(){
	// 	$(this).addClass('login-checked').siblings().removeClass('login-checked')
	// })
	$('.user-head').on('mouseover',function(event){
		event.stopPropagation();
		$(".link-2").slideDown(100);
	})
